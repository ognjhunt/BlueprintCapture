...
async function loadArkitFrameQuality(bucket, rawPrefix, tmpDir) {
...
function normalizedSceneMemoryCapture(manifest) {
    ...
    return {
        ...
        overlap_checkpoint_count: typeof raw?.overlap_checkpoint_count === "number" ? raw?.overlap_checkpoint_count : null,
...
function normalizedCaptureTopology(manifest) {
    ...
    return {
        ...
        coordinate_frame_session_id: asString(raw.coordinate_frame_session_id) ?? asString(raw.capture_session_id) ?? null,
        arkit_session_id: asString(raw.arkit_session_id) ??
...
export const extractFrames = onObjectFinalized({
    ...
    for (let i = 0; i < sortedFiles.length; i++) {
        ...
        if (pose) {
            ...
            const frameQuality = (poseFrameId ? arkitFrameQuality.get(poseFrameId) : undefined) ??
                arkitFrameQuality.get(frameId);
            if (frameQuality) {
                const sceneDepthFile = asString(frameQuality.sceneDepthFile) ?? asString(frameQuality.scene_depth_file);
                const smoothedSceneDepthFile = asString(frameQuality.smoothedSceneDepthFile) ??
                    asString(frameQuality.smoothed_scene_depth_file);
                const confidenceFile = asString(frameQuality.confidenceFile) ?? asString(frameQuality.confidence_file);
                const arkitFrame = {
                    frame_id: poseFrameId ?? frameId,
                    tracking_state: asString(frameQuality.trackingState) ?? asString(frameQuality.tracking_state) ?? null,
                    tracking_reason: asString(frameQuality.trackingReason) ?? asString(frameQuality.tracking_reason) ?? null,
                    world_mapping_status: asString(frameQuality.worldMappingStatus) ??
                        asString(frameQuality.world_mapping_status) ??
                        null,
                    relocalization_event: frameQuality.relocalizationEvent === true || frameQuality.relocalization_event === true,
                    sharpness_score: asFiniteNumber(frameQuality.sharpnessScore) ??
                        asFiniteNumber(frameQuality.sharpness_score) ??
                        null,
                    depth_source: asString(frameQuality.depthSource) ?? asString(frameQuality.depth_source) ?? null,
                    depth_valid_fraction: asFiniteNumber(frameQuality.depthValidFraction) ??
                        asFiniteNumber(frameQuality.depth_valid_fraction) ??
                        null,
                    missing_depth_fraction: asFiniteNumber(frameQuality.missingDepthFraction) ??
                        asFiniteNumber(frameQuality.missing_depth_fraction) ??
                        null,
                    anchor_observations: asStringArray(frameQuality.anchorObservations) ??
                        asStringArray(frameQuality.anchor_observations) ??
                        [],
                    exposure_duration_s: asFiniteNumber(frameQuality.exposureDurationS) ??
                        asFiniteNumber(frameQuality.exposure_duration_s) ??
                        null,
                    iso: asFiniteNumber(frameQuality.iso) ?? null,
                    exposure_target_bias: asFiniteNumber(frameQuality.exposureTargetBias) ??
                        asFiniteNumber(frameQuality.exposure_target_bias) ??
                        null,
                    white_balance_gains: asRecord(frameQuality.whiteBalanceGains) ??
    ...
    const captureDescriptor = {
        ...
        site_visit_id: asString(captureTopology?.site_visit_id) ??
            asString(captureTopology?.capture_session_id) ??
            null,
        geometry_source: null,
        geometry_ready: false,
        coordinate_frame_session_id: asString(captureTopology?.coordinate_frame_session_id) ??
