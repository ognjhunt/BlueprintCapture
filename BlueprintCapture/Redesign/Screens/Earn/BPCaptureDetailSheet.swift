...
struct BPCaptureDetailSheet: View {
    ...
    private var status: BPStatusPresentation.Entry {
        BPStatusPresentation.entry(for: detail?.status ?? entry.status)
    }
                ...
                VStack(alignment: .leading, spacing: Space.xl) {
                    ...
                    if let reason = detail?.rejectionReason, !reason.isEmpty {
                        BPProofBoundary(
                            "Review note",
                            message: reason,
                            signal: (detail?.status ?? entry.status) == .rejected ? .blocker : .caution,
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: Space.m) {
            BPRemoteFacilityImage(url: entry.thumbnailURL, height: 160)
